...
class HiperHeuristica():
    ...
    def __init__(self, instancia = Instance, V = float, low_levels = list, solucion = Solucion, tiempo = int):
        ...
        self.secuencia = []
        self.s = instancia.ub
        self.tiempo = tiempo # minutos maximos
        ...
        self.optimos = {"a1": 15,"a2": 2,"a3": 12,"a4": 3.5,"a5": 177.88,"a6": 691,"a7": 392.25,"a8": 162.94,"a9": 4.42,"a10": 17.11,"a11": 16.85,"a12": 11.25,"a13": 117.38,"a14": 181.64,"a15": 149.33,"a16": 85,"a17": 36.5,"a18": 117.2,"a19": 202,"a20": 5,"b1": 631.5,"b2": 180.64,"b3": 149.6,"b4": 155.67,"b5": 368,"b6": 83.97,"b7": 108,"b8": 227.1,"b9": 82.5,"b10": 444,"b11": 898.8,"b12": 567.64,"b13": 756.78,"b14": 885.17,"b15": 166.14, "x1": 11.67, "x2": 237}
    ...
    def implementar(self):
        self.tiempo_maximo = self.tiempo_inicio_real + self.tiempo
        n = len(self.low_levels)
        self.low_levels[n-1].instance_name = self.instancia_name
        self.low_levels[n-2].instance_name = self.instancia_name

        self.low_levels[n-1].tiempo_limite = self.tiempo_maximo
        self.low_levels[n-2].tiempo_limite = self.tiempo_maximo
        ...
        while self.tiempo_maximo - time.time() > 0:
            if round(self.mejor_sol.objective_value,2) == self.optimos[self.instancia_name] and self.mejor_sol.is_factible:
                print("Se llegó a la solución óptima")
                break
            elif round(self.mejor_sol.objective_value,2) > self.optimos[self.instancia_name] and self.mejor_sol.is_factible:
                print("se llegó a la solución óptima mejor a la encontrada por los demás")
                break
        ...
        if round(self.mejor_sol.objective_value,2) > self.optimos[self.instancia_name]:
            clave = self.instancia_name
            optimo = self.optimos[clave]
            gap = (optimo - self.mejor_sol.objective_value) / optimo
            print(f"No se llegó a la mejor solución | GAP = {gap:.2f}")
    ...
    def exportar_perfilado(self):
        ...
        filename = f"{self.nombre_excel}.xlsx"
